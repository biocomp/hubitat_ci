...
@TupleConstructor
class CapabilityInputType implements IInputType
{
    ...
    def makeInputObject(String inputName, String inputType, Object userProvidedValue) {
        ...
        def addMethod = { def name, def returnValStr, def arguments -> "${returnValStr} ${name}()" }

        builder.append(
                """
class ${capability.simpleName}_Impl extends ${capability.canonicalName}
{
    // Attributes (as properties)

    // Methods (implementaiton of the interface)
...
class DeviceInputType implements IInputType
{
    ...
    @Override
    def makeInputObject(String inputName, String inputType) {
        return null // Not sure how to mock a device
    }
}